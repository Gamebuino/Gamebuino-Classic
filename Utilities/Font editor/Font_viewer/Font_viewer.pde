...
void setup() {
  ...
  for (int x = 0; x<16; x++) {
    if (x<10)
      text(char(48+x), 13+x*13, 0);
    else
      text(char(65+x-10), 13+x*13, 0);
  }
  
  //vertical ruler
  for (int y = 0; y<8; y++) {
      text(char(48+y), 0, 18+y*18-2);
  }
  ...
  for (int y = 0; y<8; y++) {
    for (int x = 0; x<16; x++) {
      drawChar(13+x*13, 18+y*18, char(x+16*y), 2);
    }
  }
  saveFrame("char_map.png");
}
